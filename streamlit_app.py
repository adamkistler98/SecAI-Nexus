...
with col_sich:
    st.markdown(f'<div><a href="https://www.sicherheitstacho.eu/?lang=en" target="_blank" class="map-title-link">>> SICHERHEITSTACHO (DT)</a></div>', unsafe_allow_html=True)
...
with col_bit:
    st.markdown(f'<div><a href="https://threatmap.bitdefender.com/" target="_blank" class="map-title-link">>> BITDEFENDER THREAT MAP</a></div>', unsafe_allow_html=True)
    render_muted_iframe("https://threatmap.bitdefender.com/", height=1100)

st.markdown("---")

# === LARGE MAP SECTION (GREYNOISE) ===
st.markdown(f'<div><a href="https://viz.greynoise.io/trends/trending" target="_blank" class="map-title-link">>> GREYNOISE INTELLIGENCE</a></div>', unsafe_allow_html=True)
...
with l1:
    st.markdown(render_simple_link("01", "NIST AI RMF", "https://www.nist.gov/itl/ai-risk-management-framework", "Framework to manage AI-related risks to individuals and organizations."), unsafe_allow_html=True)
    st.markdown(render_simple_link("02", "HITRUST Alliance", "https://hitrustalliance.net/", "Safeguarding sensitive data and managing information risk."), unsafe_allow_html=True)
    st.markdown(render_simple_link("03", "OWASP LLM Top 10", "https://owasp.org/www-project-top-10-for-large-language-model-applications/", "Critical security risks in Large Language Models."), unsafe_allow_html=True)
    st.markdown(render_simple_link("04", "NIST CSF", "https://www.nist.gov/cyberframework", "Standards to manage and reduce cybersecurity risk."), unsafe_allow_html=True)
    st.markdown(render_simple_link("05", "ISO/IEC 27001", "https://www.iso.org/isoiec-27001-information-security.html", "International standard for ISMS management."), unsafe_allow_html=True)
    st.markdown(render_simple_link("06", "OWASP Top 10 Web", "https://owasp.org/www-project-top-ten/", "Critical web application security risks awareness."), unsafe_allow_html=True)
    st.markdown(render_simple_link("07", "NIST NVD", "https://nvd.nist.gov/", "US repository of standards-based vulnerability data."), unsafe_allow_html=True)
    st.markdown(render_simple_link("08", "CIS Controls", "https://www.cisecurity.org/controls/", "Prioritized safeguards against prevalent cyber-attacks."), unsafe_allow_html=True)
    st.markdown(render_simple_link("09", "CISA KEV Catalog", "https://www.cisa.gov/known-exploited-vulnerabilities-catalog", "Authoritative source for vulnerabilities exploited in the wild."), unsafe_allow_html=True)
    st.markdown(render_simple_link("10", "MITRE ATT&CK", "https://mitre-attack.github.io/attack-navigator/", "Matrix for mapping adversary tactics and procedures."), unsafe_allow_html=True)
    st.markdown(render_simple_link("11", "CVSS 4.0 Calc", "https://www.first.org/cvss/calculator/4.0", "Home of the Common Vulnerability Scoring System."), unsafe_allow_html=True)
    st.markdown(render_simple_link("12", "VirusTotal", "https://www.virustotal.com/", "Analyzing suspicious files, domains, IPs, and URLs."), unsafe_allow_html=True)
    st.markdown(render_simple_link("13", "Exploit Database", "https://www.exploit-db.com/", "Archive of public exploits and vulnerable software POCs."), unsafe_allow_html=True)

with l2:
    st.markdown(render_simple_link("14", "Shodan Search", "https://www.shodan.io/", "Search engine for exposed internet-connected devices."), unsafe_allow_html=True)
    st.markdown(render_simple_link("15", "Have I Been Pwned", "https://haveibeenpwned.com/", "Checking if email/domain was in a data breach."), unsafe_allow_html=True)
    st.markdown(render_simple_link("16", "Security Onion", "https://securityonionsolutions.com/", "Platform for threat hunting and enterprise security."), unsafe_allow_html=True)
    st.markdown(render_simple_link("17", "AlienVault OTX", "https://otx.alienvault.com/", "Gathering and sharing real-time crowdsourced IOCs."), unsafe_allow_html=True)
    st.markdown(render_simple_link("18", "crt.sh Search", "https://crt.sh/", "Certificate Transparency log search for attack surface mapping."), unsafe_allow_html=True)
    st.markdown(render_simple_link("19", "SANS ISC", "https://isc.sans.edu/", "Cooperative cyber threat monitor tracking network trends."), unsafe_allow_html=True)
    st.markdown(render_simple_link("20", "BleepingComputer", "https://www.bleepingcomputer.com/", "Trusted news source for tracking ransomware attacks."), unsafe_allow_html=True)
    st.markdown(render_simple_link("21", "Abuse.ch URLhaus", "https://urlhaus.abuse.ch/", "Project dedicated to tracking malware distribution sites."), unsafe_allow_html=True)
    st.markdown(render_simple_link("22", "Any.Run Sandbox", "https://any.run/", "Interactive online malware analysis sandbox."), unsafe_allow_html=True)
    st.markdown(render_simple_link("23", "URLScan.io", "https://urlscan.io/", "Analyze websites to see background executions."), unsafe_allow_html=True)
    st.markdown(render_simple_link("24", "GTFOBins", "https://gtfobins.github.io/", "List of Unix binaries used to bypass security."), unsafe_allow_html=True)
    st.markdown(render_simple_link("25", "MalwareBazaar", "https://bazaar.abuse.ch/", "Open-source repository of malware samples."), unsafe_allow_html=True)
    st.markdown(render_simple_link("26", "HackTheBox", "https://www.hackthebox.com/", "Gamified cybersecurity training playground."), unsafe_allow_html=True)

with l3:
    st.markdown(render_simple_link("27", "HackerOne", "https://www.hackerone.com/", "Bug bounty platform for elite pen testers."), unsafe_allow_html=True)
    st.markdown(render_simple_link("28", "Bugcrowd", "https://www.bugcrowd.com/", "Crowdsourced vulnerability disclosure and bounties."), unsafe_allow_html=True)
    st.markdown(render_simple_link("29", "TryHackMe", "https://tryhackme.com/", "Interactive training with hands-on virtual labs."), unsafe_allow_html=True)
    st.markdown(render_simple_link("30", "CyberChef", "https://gchq.github.io/CyberChef/", "The Cyber Swiss Army Knife for data analysis."), unsafe_allow_html=True)
    st.markdown(render_simple_link("31", "CISA Shields Up", "https://www.cisa.gov/shields-up", "Building corporate resilience against cyberattacks."), unsafe_allow_html=True)
    st.markdown(render_simple_link("32", "NIST CSRC", "https://csrc.nist.gov/", "Resource Center for framework guidelines."), unsafe_allow_html=True)
    st.markdown(render_simple_link("33", "SANS Reading Room", "https://www.sans.org/white-papers/", "Cybersecurity research documents and whitepapers."), unsafe_allow_html=True)
    st.markdown(render_simple_link("34", "DEF CON Archives", "https://defcon.org/html/links/dc-archives.html", "Presentations from the legendary hacking conference."), unsafe_allow_html=True)
    st.markdown(render_simple_link("35", "OSINT Framework", "https://osintframework.com/", "Interactive collection of OSINT gathering tools."), unsafe_allow_html=True)
    st.markdown(render_simple_link("36", "Talos Threat Intel", "https://talosintelligence.com/", "Cisco's premier threat research group."), unsafe_allow_html=True)
...
<div style="color: #555; font-size: 0.75rem; margin-bottom: 20px; padding: 0 10%; line-height: 1.4;"><b>LEGAL DISCLAIMER:</b> This dashboard is strictly for <b>educational and portfolio purposes</b>. All embedded threat maps and data sources are the property of their respective owners. SecAI-Nexus does not host, process, or claim ownership of third-party threat maps or data.<br><a rel="license" href="http://creativecommons.org/licenses/by-nc/4.0/" target="_blank" class="footer-license-link">This project's code, layout, and compilation are licensed under a Creative Commons Attribution-NonCommercial 4.0 International License (CC BY-NC 4.0).</a><br>Third-party content remains under the copyright of its original providers.</div>
<div style="color: #555; font-size: 0.8rem; margin-top: 10px;">SecAI-Nexus GRC [Version 13.7] | Cyber Threat Observability</div>
</div>"""
