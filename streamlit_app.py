...
<style>
  ...
  .cm {{background:linear-gradient(135deg,{CARD},#0d0d12);border:1px solid #1a1a2e;
    ...
    font-family:{MONO};transition:all .3s;min-height:108px;}}
  .cm:hover {{border-left-color:{GREEN};box-shadow:0 0 10px {GREEN}0d;}}
  .cm-t a {{color:{BLUE};font-size:.58rem;font-weight:bold;text-transform:uppercase;
    letter-spacing:.4px;text-decoration:none;transition:.2s;}}
  .cm-t a:hover {{color:{GREEN};text-shadow:0 0 4px {GREEN};}}
  .cm-l {{font-size:.46rem;color:{GREEN};border:1px solid {GREEN};padding:0 3px;
    margin-left:3px;vertical-align:middle;animation:pglow 3s ease-in-out infinite;}}
  .cm-e {{font-size:.46rem;color:{AMBER};border:1px solid {AMBER}80;padding:0 3px;
    margin-left:3px;vertical-align:middle;}}
  .cm-v {{color:{GREEN};font-size:1.2rem;font-weight:bold;margin:3px 0 1px;line-height:1.05;
    text-shadow:0 0 4px {GREEN}20;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
  .cm-s {{font-size:.55rem;color:#4a4a5a;margin-bottom:4px;line-height:1.15;
    white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
  .cm-d {{font-size:.56rem;border-top:1px dashed #1a1a2e;padding-top:3px;line-height:1.5;}}
  ...
  /* PULSE ROW — larger cards */
  .pulse {{background:linear-gradient(135deg,#0a0a14,#0d0d1a);border:1px solid #1a1a30;
    border-left:4px solid {CYAN};padding:12px 14px 10px;margin-bottom:8px;
    font-family:{MONO};transition:all .3s;min-height:130px;}}
  .pulse:hover {{border-left-color:{GREEN};box-shadow:0 0 14px {GREEN}12;}}
  .pulse .cm-t a {{font-size:.65rem;color:{CYAN};}}
  .pulse .cm-v {{font-size:1.45rem;color:{CYAN};text-shadow:0 0 6px {CYAN}30;}}
  .pulse .cm-s {{font-size:.6rem;}}
  .pulse .cm-d {{font-size:.6rem;}}

  .rl {{font-size:.58rem;color:#3a3a4a;text-transform:uppercase;letter-spacing:1px;
  ...
  .rl-pulse {{font-size:.62rem;color:{CYAN};text-transform:uppercase;letter-spacing:1.2px;
...
def fetch_bazaar():
    ...
    try:
        ...
        return {"d1":d1,"d7":d7,"total":len(lines),"tf":tf,
                "families":len(sm),"top3":sorted(sm,key=sm.get,reverse=True)[:3]}
    except: return None
...
def fetch_feodo():
    ...
    try:
        ...
        # extract malware families from column
        mw={}
        for l in lines:
            parts=l.split(",")
            if len(parts)>=5:
                fam=parts[4].strip().strip('"')
                if fam and fam!="malware": mw[fam]=mw.get(fam,0)+1
        top_mw=max(mw,key=mw.get) if mw else "N/A"
        return {"on":on,"off":off,"total":len(lines),"top_mw":top_mw,"mw_count":mw.get(top_mw,0)}
    except: return None
...
# ── SANS ISC TOP PORTS (live attack data!) ────────────────────────────────────
@st.cache_data(ttl=1800, show_spinner=False)
def fetch_topports():
    ...
    try:
        ...
        if isinstance(data, list) and len(data)>0:
            ports = []
            for p in data[:5]:
                ports.append({"port":p.get("targetport","?"),
                              "records":int(p.get("records",0)),
                              "sources":int(p.get("sources",0)),
                              "targets":int(p.get("targets",0))})
            return {"ports":ports, "total_records":sum(p["records"] for p in ports)}
    except: pass
    return None
...
def fetch_topips():
    ...
    try:
        data = r.json()
        if isinstance(data, list) and len(data)>0:
            total_attacks = sum(int(ip.get("count",0)) for ip in data[:5])
            total_sources = len(data)
            return {"total_attacks":total_attacks, "top_ip":data[0].get("ip","?"),
                    "top_count":int(data[0].get("count",0)), "sources":total_sources}
    except: pass
    return None

# ── SANS Webhoneypot Summary ──────────────────────────────────────────────────
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_honeypot():
        ...
        if r:
            try:
                ...
                if isinstance(data, dict) and "reports" in data:
                    return {"reports":int(data.get("reports",0)),
                            "targets":int(data.get("targets",0)),
                            "sources":int(data.get("sources",0)),
                            "date":dt}
                elif isinstance(data, list) and len(data)>0:
                    d = data[0]
                    return {"reports":int(d.get("reports",0)),
                            "targets":int(d.get("targets",0)),
                            "sources":int(d.get("sources",0)),
                            "date":dt}
            except: pass
    return None
...
PORT_NAMES = {22:"SSH",23:"Telnet",25:"SMTP",53:"DNS",80:"HTTP",110:"POP3",
    123:"NTP",135:"MSRPC",137:"NetBIOS",139:"SMB",143:"IMAP",443:"HTTPS",
    445:"SMB",993:"IMAPS",1433:"MSSQL",1883:"MQTT",3306:"MySQL",3389:"RDP",
    5060:"SIP",5432:"Postgres",5900:"VNC",6379:"Redis",8080:"HTTP-Alt",
    8443:"HTTPS-Alt",8888:"HTTP-Alt",27017:"MongoDB"}
def port_name(p):
    try: return PORT_NAMES.get(int(p), f"Port {p}")
    except: return str(p)
...
def card(title, url, value, subtitle, d30, d30c, d1yr, d1yc, live=True):
  ...
  <div class="cm-v">{value}</div><div class="cm-s">{subtitle}</div>
  <div class="cm-d"><span style="color:#2a2a3a;">30d </span><span class="{d30c}">{d30}</span>
    <span style="color:#2a2a3a;"> 1yr </span><span class="{d1yc}">{d1yr}</span></div></div>""", unsafe_allow_html=True)

def pulse_card(title, url, value, subtitle, d30, d30c, d1yr, d1yc, live=True):
    b = f'<span class="cm-l">LIVE</span>' if live else f'<span class="cm-e">EST</span>'
    st.markdown(f"""<div class="pulse">
  <div class="cm-t"><a href="{url}" target="_blank">{title}</a>{b}</div>
  <div class="cm-v">{value}</div><div class="cm-s">{subtitle}</div>
  <div class="cm-d"><span style="color:#2a2a3a;">30d </span><span class="{d30c}">{d30}</span>
    <span style="color:#2a2a3a;"> 1yr </span><span class="{d1yc}">{d1yr}</span></div></div>""", unsafe_allow_html=True)
...
def lcard(title, url, data, vf, sf, d30f, d1yf, d30c="d-b", d1yc="d-b", fsub="awaiting"):
    if data:
        try: card(title,url,vf(data),sf(data),d30f(data),d30c,d1yf(data),d1yc,True); return
        except: pass
  ...
  <div class="cm-s">{fsub}</div><div class="cm-d" style="color:#2a2a3a;">Cloud deploy</div></div>""", unsafe_allow_html=True)

def lpulse(title, url, data, vf, sf, d30f, d1yf, d30c="d-b", d1yc="d-b", fsub="awaiting"):
    if data:
        try: pulse_card(title,url,vf(data),sf(data),d30f(data),d30c,d1yf(data),d1yc,True); return
        except: pass
    st.markdown(f"""<div class="pulse" style="opacity:.5;">
  <div class="cm-t"><a href="{url}" target="_blank">{title}</a><span class="cm-l">LIVE</span></div>
  <div class="cm-v" style="font-size:1rem;color:#2a7a3a;">Syncing…</div>
  <div class="cm-s">{fsub}</div><div class="cm-d" style="color:#2a2a3a;">DShield sensor network</div></div>""", unsafe_allow_html=True)
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-end;flex-wrap:wrap;">
    <div>
      ...
      <span style="font-size:.48rem;color:#3a3a4a;border:1px solid #2a2a3a;
        padding:1px 4px;margin-left:5px;vertical-align:middle;">v19.0</span></div>
    <div style="text-align:right;">
      ...
      <div style="font-size:.55rem;color:#3a3a4a;margin-top:1px;">
        <span class="sd sg"></span>FEEDS NOMINAL · 51 INDICATORS · 9 MAPS · 52 RESOURCES</div></div>
...
# ── EST BASELINES (all verified against published reports) ────────────────────
CVE_TOT=29_000; CVE_CRIT=4_200; CVE_HIGH=12_500  # NVD 2024
RANSOM=5_500; SUPPLY=3_000; INSIDER=6_800          # CrowdStrike GTR / DBIR 2024
BREACH=8_000_000_000; BEC=21_489; PHISH=1_970_000 # DBIR / IC3 / APWG
IDENTITY=17_000_000_000                            # SpyCloud 2024
GDPR=2_100_000_000; IC3LOSS=12_500_000_000         # DLA Piper / IC3
DDOS=15_400_000; IOT_MAL=112_000_000               # Cloudflare / SonicWall
CRYPTO=2_200_000_000                               # Chainalysis 2024: $2.2B stolen
CISA_ADV_YR=850; CISA_ICS_YR=420                   # CISA 2024 advisory totals (est.)
RECOVERY=2_730_000                                 # Sophos 2024 avg recovery cost
...
<div style="margin:2px 0 10px;">
  ...
  <span style="font-size:.6rem;color:#3a3a4a;">
    ...
    &ensp;<span class="sd sc"></span><span style="color:{CYAN};">PULSE</span> = DShield sensor network
    &ensp;51 indicators across 6 categories
  </span></div>""", unsafe_allow_html=True)

# ─── ROW 1: VULNERABILITY & EXPLOIT INTEL (9 cards) ──────────────────────────
...
with c[1]:
    lcard("KEV RANSOMWARE","https://www.cisa.gov/known-exploited-vulnerabilities-catalog",
        kev, lambda d:_f(d["rw"]), lambda d:"Ransomware-linked CVEs",
...
# ─── ROW 2: MALWARE, C2 & ADVISORIES (9 cards) ──────────────────────────────
...
with c[5]:
    card("CISA ALL ADV","https://www.cisa.gov/news-events/cybersecurity-advisories",
        f"~{per(CISA_ADV_YR,7)}/wk", f"~{CISA_ADV_YR}/yr · AA+ICS+MA",
        f"+{per(CISA_ADV_YR,30)}","d-b", f"~{CISA_ADV_YR}","d-b", False)
with c[6]:
    card("ICS/SCADA ADV","https://www.cisa.gov/news-events/cybersecurity-advisories/ics-advisories",
        f"~{per(CISA_ICS_YR,7)}/wk", f"~{CISA_ICS_YR}/yr · OT/ICS",
        f"+{per(CISA_ICS_YR,30)}","d-b", f"~{CISA_ICS_YR}","d-b", False)
with c[7]:
    card("RANSOMWARE RATE","https://www.sophos.com/en-us/blog/the-state-of-ransomware-2024",
        "59%", "of orgs hit · Sophos",
        "–","d-n", "-7% YoY","d-g", False)
with c[8]:
...
# ─── ROW 3: BREACH, INCIDENT & COST (9 cards) ────────────────────────────────
...
with c[8]:
    card("AI-POWERED ATTKS","https://www.crowdstrike.com/global-threat-report/",
        "+150% YoY", "vishing/deepfake · CS",
        "–","d-n", "+150%","d-b", False)

# ─── ROW 4: FINANCIAL & EMERGING (9 cards) ────────────────────────────────────
...
with c[8]:
    card("ILLICIT CRYPTO","https://www.chainalysis.com/blog/2025-crypto-crime-report-introduction/",
        "$40.1B", "total 2024 · Chainalysis",
        "–","d-n", "$40.1B","d-b", False)

# ─── ROW 5: POSTURE & WORKFORCE (9 cards) ────────────────────────────────────
rl("▸ SECURITY POSTURE, DETECTION & WORKFORCE  [EST]")
c = st.columns(9)
with c[0]:
...
with c[8]:
    lcard("KEV VENDORS","https://www.cisa.gov/known-exploited-vulnerabilities-catalog",
        ...
        lambda d:"–", lambda d:f'{d["vendors"]}', d30c="d-n", d1yc="d-b", fsub="KEV vendor count")

# ─── ROW 6: LIVE THREAT PULSE — SANS DSHIELD SENSOR DATA ─────────────────────
st.markdown(f'<div class="rl-pulse">⚡ LIVE THREAT PULSE — SANS DSHIELD SENSOR NETWORK (REAL-TIME ATTACK DATA)</div>', unsafe_allow_html=True)
c = st.columns(6)
with c[0]:
    lpulse("#1 ATTACKED PORT","https://isc.sans.edu/",
        topports,
        lambda d: f'{port_name(d["ports"][0]["port"])} ({d["ports"][0]["port"]})',
        lambda d: f'{_f(d["ports"][0]["records"])} events · {_f(d["ports"][0]["sources"])} sources',
        lambda d:"–", lambda d:f'{_f(d["total_records"])} total',
        d30c="d-n", d1yc="d-b", fsub="SANS ISC Top Ports API")
with c[1]:
    lpulse("#2 ATTACKED PORT","https://isc.sans.edu/",
        topports,
        lambda d: f'{port_name(d["ports"][1]["port"])} ({d["ports"][1]["port"]})' if len(d["ports"])>1 else "–",
        lambda d: f'{_f(d["ports"][1]["records"])} events' if len(d["ports"])>1 else "–",
        lambda d:"–", lambda d:"–",
        d30c="d-n", d1yc="d-n", fsub="SANS ISC Top Ports")
with c[2]:
    lpulse("#3 ATTACKED PORT","https://isc.sans.edu/",
        topports,
        lambda d: f'{port_name(d["ports"][2]["port"])} ({d["ports"][2]["port"]})' if len(d["ports"])>2 else "–",
        lambda d: f'{_f(d["ports"][2]["records"])} events' if len(d["ports"])>2 else "–",
        lambda d:"–", lambda d:"–",
        d30c="d-n", d1yc="d-n", fsub="SANS ISC Top Ports")
with c[3]:
    lpulse("TOP ATTACK SOURCE","https://isc.sans.edu/",
        topips,
        lambda d: d["top_ip"],
        lambda d: f'{_f(d["top_count"])} packets blocked',
        lambda d:"–", lambda d:f'{_f(d["total_attacks"])} from top 5',
        d30c="d-n", d1yc="d-b", fsub="SANS ISC Top IPs API")
with c[4]:
    lpulse("HONEYPOT HITS","https://isc.sans.edu/",
        honeypot,
        lambda d: _f(d["reports"]),
        lambda d: f'{d["sources"]} sources · {d["targets"]} targets',
        lambda d:"–", lambda d:f'{d.get("date","today")}',
        d30c="d-n", d1yc="d-n", fsub="SANS webhoneypot summary")
with c[5]:
    if topports:
        try:
            top5 = ", ".join([f'{port_name(p["port"])}' for p in topports["ports"][:5]])
            pulse_card("TOP 5 PORTS ATTACKED","https://isc.sans.edu/",
                top5,
                f'{_f(topports["total_records"])} total events from DShield sensors',
                "–","d-n", f'{_f(topports["total_records"])}', "d-b", True)
        except:
            pulse_card("DSHIELD SENSORS","https://isc.sans.edu/",
                "Active","Global distributed sensor network",
                "–","d-n", "24/7","d-g", True)
    else:
        st.markdown(f"""<div class="pulse" style="opacity:.5;">
  <div class="cm-t"><a href="https://isc.sans.edu/" target="_blank">TOP 5 PORTS</a><span class="cm-l">LIVE</span></div>
  <div class="cm-v" style="font-size:1rem;color:#2a7a3a;">Syncing…</div>
  <div class="cm-s">DShield sensor network</div><div class="cm-d" style="color:#2a2a3a;">Cloud deploy</div></div>""", unsafe_allow_html=True)
...
<div class="sb">
  ...
  <br><span style="color:#3a3a4a;font-weight:bold;text-transform:uppercase;letter-spacing:.4px;">
    <span class="sd sc"></span>PULSE&nbsp;</span>
  <a href="https://isc.sans.edu/api/topports/records/5?json" target="_blank" class="sl">DShield TopPorts</a> ·
...
<div style="margin:4px 0 10px;">
  ...
  <span style="font-size:.55rem;color:#3a3a4a;margin-left:8px;">9 REAL-TIME SOURCES</span></div>""", unsafe_allow_html=True)
...
<div style="margin-top:24px;margin-bottom:14px;text-align:center;">
  ...
  <div style="font-size:.55rem;color:#3a3a4a;margin-top:2px;">52 CURATED · FRAMEWORKS · TOOLS · TRAINING · INTEL</div></div>""", unsafe_allow_html=True)
...
<div style="border-top:1px solid #141420;padding-top:20px;margin-top:32px;text-align:center;font-family:{MONO};">
  ...
  <div style="color:#1a1a2a;font-size:.65rem;">
    SecAI-Nexus GRC [v19.0] · Live Data Engine ·
    51 Indicators · 11 Live Feeds · 9 Maps · 52 Resources · {now_utc.strftime("%Y")}</div></div>""", unsafe_allow_html=True)
