...
# === ADDITIONAL GRC RESOURCES (TOP 20 - CONSOLIDATED) ===
...
with link_col1:
    ...
    st.markdown(render_simple_link("04", "MITRE ATT&CK", "https://mitre-attack.github.io/attack-navigator/", "The industry-standard matrix for mapping adversary tactics, techniques, and procedures (TTPs)."), unsafe_allow_html=True)
    ...
    st.markdown(render_simple_link("08", "AlienVault OTX", "https://otx.alienvault.com/", "The largest open community for gathering and sharing real-time crowdsourced Indicators of Compromise (IOCs)."), unsafe_allow_html=True)
    st.markdown(render_simple_link("09", "OWASP Top 10 Project", "https://owasp.org/www-project-top-ten/", "Foundational awareness document for critical web application security risks."), unsafe_allow_html=True)
    st.markdown(render_simple_link("10", "Security Onion", "https://securityonionsolutions.com/", "Free and open-source platform for threat hunting and enterprise security monitoring."), unsafe_allow_html=True)

with link_col2:
    st.markdown(render_simple_link("11", "Abuse.ch URLhaus", "https://urlhaus.abuse.ch/", "Open project dedicated to sharing and tracking malware distribution sites and payloads."), unsafe_allow_html=True)
    st.markdown(render_simple_link("12", "SANS Internet Storm Center", "https://isc.sans.edu/", "Global cooperative cyber threat monitor tracking network anomalies and port trends."), unsafe_allow_html=True)
    st.markdown(render_simple_link("13", "BleepingComputer", "https://www.bleepingcomputer.com/", "Premier, trusted news source for tracking ransomware attacks and daily cyber events."), unsafe_allow_html=True)
    st.markdown(render_simple_link("14", "Any.Run Sandbox", "https://any.run/", "Interactive online malware analysis sandbox allowing researchers to safely observe payloads."), unsafe_allow_html=True)
    st.markdown(render_simple_link("15", "FIRST CVSS Calculator", "https://www.first.org/cvss/calculator/4.0", "The official home of the Common Vulnerability Scoring System for calculating environmental risk."), unsafe_allow_html=True)
    st.markdown(render_simple_link("16", "NIST Cybersecurity Framework", "https://www.nist.gov/cyberframework", "Voluntary guidance based on standards to better manage and reduce cybersecurity risk."), unsafe_allow_html=True)
    st.markdown(render_simple_link("17", "ISO/IEC 27001 Standard", "https://www.iso.org/isoiec-27001-information-security.html", "International standard for establishing Information Security Management Systems (ISMS)."), unsafe_allow_html=True)
