# Security Policy

## Supported Versions
Currently, only the latest active branch of the SecAI-Nexus GRC dashboard receives security updates.

| Version | Supported          |
| ------- | ------------------ |
| 13.7    | :white_check_mark: |

## Reporting a Vulnerability
We take the security of SecAI-Nexus seriously. If you discover a security vulnerability within this project, please do NOT disclose it publicly on the issue tracker or social media. 

Instead, please report it directly and privately to the project maintainer, **Adam Kistler**, via LinkedIn direct message.

**Please include the following details in your confidential report:**
* A clear description of the vulnerability and its potential impact.
* Step-by-step instructions to reproduce the issue.
* Potential mitigation strategies or architectural recommendations (if known).

All legitimate security disclosures will be investigated and addressed promptly. Thank you for helping keep the open-source community secure!
